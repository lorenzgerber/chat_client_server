...
io_handler* create_client_tcp_socket(char *server_name, int port){
	...
	io->socket = setup_tcp_send_socket();
...
int tcp_client_send_pdu(struct io_handler *self, pdu* pdu){
	...
	if(sendto(self->socket, MBA->array, pdu->get_message_length(pdu), 0, (struct sockaddr *)self->hints->ai_addr, self->hints->ai_addrlen)<0){
